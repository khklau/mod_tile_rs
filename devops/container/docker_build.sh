...
if [ -z "$(docker image ls | grep '^aus-osm-postgis' | grep '13-3.4')" ]; then
    ...
    docker container run --name osm-postgis --rm --network=osm-local --detach -p 5432:5432 osm-postgis:13-3.4 \
        && sleep 10 \
...
if [ ! -e ${CONTAINER_DIR}/legacy-renderer/style.xml ]; then
    docker container run --name aus-osm-postgis --rm --network=osm-local --detach -p 5432:5432 aus-osm-postgis:13-3.4
    sleep 10
    docker container run --name stylesheet-generation --rm --network=osm-local stylesheet-generation:5.3.1 \
